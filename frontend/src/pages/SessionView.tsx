...
socketService.onUserJoined((newUser: any) => {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            const blob = await sessionService.exportSessionPDF(session._id);
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Session-${session.code}-Export.pdf`);
    ...
    const exportToCSV = async () => {
        if (!session) return;
        try {
            setToast({ message: 'Generating CSV...', type: 'info' });
            const blob = await sessionService.exportSessionCSV(session._id);
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Session-${session.code}-Export.csv`);
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
